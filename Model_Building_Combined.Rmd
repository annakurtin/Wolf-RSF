...
# dredging
dredge_bv1 <- dredge(bv_full1)
head(dredge_bv1, n = 10)
bv1Top_dredge <- glm(used~deer_w2 + DistFromHighHumanAccess2 +  goat_w2 + sheep_w2 + closedConif + modConif + burn, family = binomial(logit), data = bvkde)
# AIC 893.3
plot(dredge_bv1)
...
# stepwide model selection forwards
...
# dredging
dredge_bv2 <- dredge(bv_full2)
head(dredge_bv2, n = 10)
bv2Top_dredge <- glm(formula = used ~ deer_w2 + Elevation2 + sheep_w2 + openConif + 
...
# had a worse AIC
vif(bv2Top_dredge)
# VARIABLES WITH ISSUES: openConif, modConif, closedConif
...
# AIC 397.2
vif(rd1Top_forward) # this looks good 
# VARIABLES WITH ISSUES: burn, openConif, shrub, closedConif, modConif

# dredging
dredge_rd1 <- dredge(rd_full1)
head(dredge_rd1, n = 10)
...
rd1Top_dredge2 <- glm(formula = used ~ elk_w2 + DistFromHumanAccess2 + goat_w2 + 
    ...
    shrub + burn + alpine, family = binomial(logit), data = rdkde)
vif(rd1Top_dredge2)
# VARIABLES WITH ISSUES: openConif, modConif, closedConif, shrub, bun
# subset the top models with dAIC < 2
